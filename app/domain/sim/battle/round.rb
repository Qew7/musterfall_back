  ...
  module Battle
    module Round
      ...
      def play(battle:, round_number:, rng:)
        ...
        round[:events].concat(State.apply_faction_passives!(battle[:sides][:left], terrain: terrain))
        round[:events].concat(State.apply_faction_passives!(battle[:sides][:right], terrain: terrain))
